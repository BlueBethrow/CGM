...
public:
  ...
  GLApp(512, 512, 1, "Volume Exercise 1", true, false, false, args)
  {
  }
  ...
  void generateVolume() {
    const AnalyticVolumeType type = volumeTypes[currentVolume];
    volume = Volume::analytic(type);
    std::cout << "Generated volume " << currentVolume + 1 << "/"
              << volumeTypes.size() << ": " << volumeName(type) << std::endl;
  ...
  void updateTransferFunction() {
    std::vector<Vec4> data(transferFunction.getSize());
    for (size_t i = 0; i < data.size(); ++i) {
      const float x = data.size() > 1
        ? float(i) / float(data.size() - 1)
        : 0.0f;
      ...
      data[i] = Vec4{x, x, x, opacity};
    }
    transferFunction.setData(data);
  }
  ...
  void setupVolumes() {
    volumeTypes = {
      AnalyticVolumeType::Sphere,
      AnalyticVolumeType::Waves,
      AnalyticVolumeType::Shell
    };
  }
    ...
    if (action == GLENV_PRESS) {
      switch (key) {
        case GLENV_KEY_ESCAPE :
          ...
          break;
        case GLENV_KEY_V:
          currentVolume = (currentVolume + 1) % volumeTypes.size();
          generateVolume();
          stepStart = 0.10f;
          stepWidth = 0.25f;
          updateTransferFunction();
...
private:
  ...
  std::vector<AnalyticVolumeType> volumeTypes;
  size_t currentVolume{0};
  float stepStart{0.10f};
  float stepWidth{0.25f};
  float alphaRisePower{1.0f};
  float peakOpacity{0.35f};
